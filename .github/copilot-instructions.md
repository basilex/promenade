...
Use this file as a concise, high-signal guide for working in Promenade. See [README.md](README.md) and [docs/INDEX.md](docs/INDEX.md) for full docs.

## Big picture
- DDD with strict bounded contexts under [internal/contexts](internal/contexts) (no cross-context imports). Cross-context communication is via the Event Bus in [pkg/bus](pkg/bus/README.md).
- Dependency wiring order lives in [cmd/api/bootstrap.go](cmd/api/bootstrap.go) (logger → DB → migrations → Redis/cache → JWT → Event Bus → integrations → routers).
- Multi-database support is deliberate: PostgreSQL prod, SQLite dev/test. Use TEXT for UUID/JSON in migrations (see [migrations/README.md](migrations/README.md)).

## Critical workflows
- Workspace state is in [.promenade.workspace](.promenade.workspace.example). Use `make workspace`, `make switch-postgres-dev`, `make switch-sqlite-dev`, `make dev`, `make dev-fresh` (see [Makefile](Makefile)).
...
- IDs: always `uuidv7.New()` from [pkg/uuidv7/uuidv7.go](pkg/uuidv7/uuidv7.go); do not use UUID v4.
- Aggregates must embed `aggregate.BaseAggregate` and use `Touch()` and `GetID()` (see [pkg/aggregate/README.md](pkg/aggregate/README.md)).
- Use cases: return domain error constants from errors.go; no inline `fmt.Errorf` in usecase.go. Handlers map errors with `errors.Is()` and expose generic system messages (see [docs/guides/security-patterns.md](docs/guides/security-patterns.md)).
- Repositories embed BaseRepository and use `getExecutor(ctx)` for transaction propagation (see any adapter in [internal/contexts](internal/contexts)).
- JSON fields: use `jsonstore.Field[T]` from [pkg/jsonstore/README.md](pkg/jsonstore/README.md); do not manual marshal/unmarshal.

## Integration points
- Event Bus topics/constants live in [pkg/bus](pkg/bus/topics.go); publish after state changes and log errors without failing the operation.
- Fiscal context integrates Checkbox via [pkg/fiscal/checkbox](pkg/fiscal/checkbox) and registers order event handlers in [cmd/api/bootstrap.go](cmd/api/bootstrap.go) and [internal/contexts/fiscal/integration](internal/contexts/fiscal/integration).
...
## Examples to copy
- Warehouse order/inventory event flow in [internal/contexts/warehouse/integration](internal/contexts/warehouse/integration) (order.confirmed → reserve, order.cancelled → release, order.fulfilled → commit).
- HTTP handler error mapping style in any handler under [internal/contexts](internal/contexts) (validation errors return details, system errors are generic).
