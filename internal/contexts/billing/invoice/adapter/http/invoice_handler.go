...
import (
	...
	"github.com/basilex/promenade/internal/contexts/billing/invoice/dto"
	"github.com/basilex/promenade/internal/contexts/billing/invoice/usecase"
...
type InvoiceHandler struct {
	useCase usecase.IInvoiceUseCase
}

// NewInvoiceHandler creates a new invoice handler
func NewInvoiceHandler(uc usecase.IInvoiceUseCase) *InvoiceHandler {
...
// @Accept json
// @Produce json
// @Param request body dto.CreateInvoiceRequest true "Invoice creation request"
// @Success 201 {object} response.Response{data=dto.InvoiceResponse}
...
func (h *InvoiceHandler) Create(c *gin.Context) {
	...
	customerID, err := dto.ParseCustomerID(req.CustomerID)
	if err != nil {
		response.ErrorResponse(c, http.StatusBadRequest, "INVALID_CUSTOMER_ID", err.Error())
		return
	}

	orderID, err := dto.ParseOrderID(req.OrderID)
	if err != nil {
	...
	response.Created(c, dto.ToInvoiceResponse(inv))
}
...
// @Success 200 {object} response.Response{data=dto.InvoiceResponse}
// @Failure 400 {object} response.Response
...
func (h *InvoiceHandler) GetByID(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, invoiceerrors.ErrInvoiceNotFound) {
	...
	response.Success(c, dto.ToInvoiceResponse(inv))
}
...
// @Produce json
// @Param id path string true "Invoice ID (UUID)"
// @Param request body dto.AddLineItemRequest true "Line item request"
// @Success 200 {object} response.Response{data=dto.InvoiceResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
...
func (h *InvoiceHandler) AddLineItem(c *gin.Context) {
	...
	response.Success(c, dto.ToInvoiceResponse(updatedInv))
}
...
func (h *InvoiceHandler) RemoveLineItem(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, invoiceerrors.ErrInvoiceLineNotFound) {
...
// @Param id path string true "Invoice ID (UUID)"
// @Param lineId path string true "Line Item ID (UUID)"
// @Param request body dto.UpdateLineItemRequest true "Update request"
// @Success 200 {object} response.Response{data=dto.InvoiceResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
...
// @Produce json
// @Param id path string true "Invoice ID (UUID)"
// @Param request body dto.MarkAsPaidRequest true "Payment date"
// @Success 200 {object} response.Response{data=dto.InvoiceResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
...
func (h *InvoiceHandler) MarkAsPaid(c *gin.Context) {
	...
	paidDate, err := dto.ParsePaidDate(req.PaidDate)
	if err != nil {
...
// @Produce json
// @Param id path string true "Invoice ID (UUID)"
// @Param request body dto.UpdateTaxRequest true "Tax amount"
// @Success 200 {object} response.Response{data=dto.InvoiceResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
...
func (h *InvoiceHandler) List(c *gin.Context) {
	...
	// Apply filters
	if req.CustomerID != "" {
		customerID, parseErr := dto.ParseCustomerID(req.CustomerID)
		if parseErr != nil {
		...
		orderID, parseErr := dto.ParseCustomerID(req.OrderID)
		if parseErr != nil {
		...
		status := aggregate.InvoiceStatus(req.Status)
		var tmpTotal int
	...
	response.SuccessWithPagination(c, dto.ToInvoiceListResponse(invoices), total, req.Page, req.PageSize)
}
...
func (h *InvoiceHandler) ListOverdue(c *gin.Context) {
	...
	response.SuccessWithPagination(c, dto.ToInvoiceListResponse(invoices), int64(tmpTotal), req.Page, req.PageSize)
}
