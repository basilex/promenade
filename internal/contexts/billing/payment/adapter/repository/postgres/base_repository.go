...
// getExecutor returns the appropriate executor (tx or db) from context
...
func (r *BaseRepository) NamedExec(ctx context.Context, query string, args ...interface{}) (sql.Result, error) {
	executor := r.getExecutor(ctx)
	return executor.ExecContext(ctx, query, args...)
}
