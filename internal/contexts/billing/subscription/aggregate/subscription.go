...
func NewSubscription(
	...
	if customerID == uuidv7.Nil {
		return nil, subscriptionerrors.ErrCustomerIDRequired
	}
	if planID == "" {
		return nil, subscriptionerrors.ErrPlanIDRequired
	}
	if currency == "" {
		return nil, subscriptionerrors.ErrCurrencyRequired
	}
	if amount <= 0 {
		return nil, subscriptionerrors.ErrAmountMustBePositive
	}

	money, err := valueobject.NewMoney(amount, currency)
	if err != nil {
		return nil, subscriptionerrors.ErrInvalidMoney
	}
...
func (s *Subscription) Activate() error {
	if s.Status != SubscriptionStatusTrial && s.Status != SubscriptionStatusPaused {
		return subscriptionerrors.ErrCannotActivate
	}
...
func (s *Subscription) Pause() error {
	if s.Status != SubscriptionStatusActive {
		return subscriptionerrors.ErrCanOnlyPauseActive
	}
...
func (s *Subscription) Resume() error {
	if s.Status != SubscriptionStatusPaused {
		return subscriptionerrors.ErrCanOnlyResumePaused
	}
...
func (s *Subscription) Cancel(reason string, effectiveDate time.Time) error {
	if s.Status == SubscriptionStatusCancelled || s.Status == SubscriptionStatusExpired {
		return subscriptionerrors.ErrAlreadyInTerminalStatus
	}
...
func (s *Subscription) Renew() error {
	if s.Status != SubscriptionStatusActive {
		return subscriptionerrors.ErrCanOnlyRenewActive
	}
...
func (s *Subscription) Expire() error {
	if s.Status == SubscriptionStatusExpired {
		return subscriptionerrors.ErrAlreadyExpired
	}
...
func (s *Subscription) Validate() error {
	if s.CustomerID == uuidv7.Nil {
		return subscriptionerrors.ErrCustomerIDRequired
	}
	if s.PlanID == "" {
		return subscriptionerrors.ErrPlanIDRequired
	}
	if s.Amount.Amount <= 0 {
		return subscriptionerrors.ErrAmountMustBePositive
	}
	if s.StartDate.IsZero() {
		return subscriptionerrors.ErrStartDateRequired
	}
	if s.RenewalDate.Before(s.StartDate) {
		return subscriptionerrors.ErrRenewalDateInvalid
	}
	return nil
}
