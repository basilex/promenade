...
func TestNewCashRegister(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			model:          "Checkbox",
			createdBy:      createdBy,
			wantErr:        cashregistererrors.ErrOrganizationIDRequired,
		},
		{
			...
			model:          "Checkbox",
			createdBy:      createdBy,
			wantErr:        cashregistererrors.ErrFiscalNumberRequired,
		},
		{
			name:           "missing model",
			organizationID: orgID,
			...
			model:          "",
			createdBy:      createdBy,
			wantErr:        cashregistererrors.ErrModelRequired,
		},
		{
			...
			wantErr:        cashregistererrors.ErrCreatedByRequired,
		},
	}
...
func TestCashRegister_Activate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			licenseKey: "",
			setup:      func() {},
			wantErr:    cashregistererrors.ErrLicenseKeyRequired,
		},
		{
			name:       "already active",
			...
			wantErr: cashregistererrors.ErrCashRegisterAlreadyActive,
		},
	}
...
func TestCashRegister_Deactivate(t *testing.T) {
	...
	err = cr.Deactivate(deactivatedBy)
	assert.Error(t, err)
	assert.ErrorIs(t, err, cashregistererrors.ErrCashRegisterAlreadyInactive)
}
...
func TestCashRegister_OpenShift(t *testing.T) {
	...
	assert.ErrorIs(t, err, cashregistererrors.ErrShiftIDRequired)
	...
	assert.ErrorIs(t, err, cashregistererrors.ErrShiftAlreadyOpen)
}
...
func TestCashRegister_CloseShift(t *testing.T) {
	...
	assert.ErrorIs(t, err, cashregistererrors.ErrNoActiveShift)
...
func TestCashRegister_Validate_Errors(t *testing.T) {
	cr := &CashRegister{}
	assert.ErrorIs(t, cr.Validate(), cashregistererrors.ErrOrganizationIDRequired)

	cr.OrganizationID = uuidv7.New()
	assert.ErrorIs(t, cr.Validate(), cashregistererrors.ErrFiscalNumberRequired)

	cr.FiscalNumber = "123"
	assert.ErrorIs(t, cr.Validate(), cashregistererrors.ErrModelRequired)
}
