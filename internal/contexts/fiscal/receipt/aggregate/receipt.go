...
func NewReceipt(cashRegisterID, orderID uuidv7.UUID, paymentType PaymentType, receiptType ReceiptType, currency string, lines []ReceiptLine, createdBy uuidv7.UUID) (*Receipt, error) {
	if cashRegisterID == uuidv7.Nil {
		return nil, receipterrors.ErrCashRegisterIDRequired
	}
	if orderID == uuidv7.Nil {
		return nil, receipterrors.ErrOrderIDRequired
	}
	if paymentType == "" {
		return nil, receipterrors.ErrPaymentTypeRequired
	}
	if receiptType == "" {
		return nil, receipterrors.ErrReceiptTypeRequired
	}
	if currency == "" {
		return nil, receipterrors.ErrCurrencyRequired
	}
	if createdBy == uuidv7.Nil {
		return nil, receipterrors.ErrCreatedByRequired
	}
	if len(lines) == 0 {
		return nil, receipterrors.ErrReceiptLineNameRequired
	}
...
func (r *Receipt) MarkPrinted(fiscalNumber, fiscalURL, qrCode string, printedBy uuidv7.UUID) error {
	if r.Status == ReceiptStatusPrinted {
		return receipterrors.ErrReceiptAlreadyPrinted
	}
	if r.Status == ReceiptStatusCancelled {
		return receipterrors.ErrReceiptAlreadyCancelled
	}
	if fiscalNumber == "" {
		return receipterrors.ErrFiscalNumberRequired
	}
...
func (r *Receipt) Cancel(reason string, cancelledBy uuidv7.UUID) error {
	...
	if reason == "" {
		return receipterrors.ErrReceiptCancelReasonRequired
	}
...
func (r *Receipt) Validate() error {
	if r.CashRegisterID == uuidv7.Nil {
		return receipterrors.ErrCashRegisterIDRequired
	}
	if r.OrderID == uuidv7.Nil {
		return receipterrors.ErrOrderIDRequired
	}
	if r.PaymentType == "" {
		return receipterrors.ErrPaymentTypeRequired
	}
	if r.ReceiptType == "" {
		return receipterrors.ErrReceiptTypeRequired
	}
	if r.Currency == "" {
		return receipterrors.ErrCurrencyRequired
	}

	lines := r.Lines.Get()
	if len(lines) == 0 {
		return receipterrors.ErrReceiptLineNameRequired
	}
...
func calculateTotals(lines []ReceiptLine) ([]ReceiptLine, int64, int64, error) {
	if len(lines) == 0 {
		return nil, 0, 0, receipterrors.ErrReceiptLineNameRequired
	}
	...
	for i, line := range lines {
		...
		if line.Quantity <= 0 {
			return nil, 0, 0, receipterrors.ErrReceiptLineQuantityInvalid
		}
		if line.PriceCents < 0 {
			return nil, 0, 0, receipterrors.ErrReceiptLinePriceInvalid
		}
		if line.TaxRate < 0 || line.TaxRate > 100 {
			return nil, 0, 0, receipterrors.ErrReceiptLineTaxRateInvalid
		}
