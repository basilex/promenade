...
func TestToUserResponse(t *testing.T) {
	u, err := aggregate.NewUser("test@example.com", "hashedPassword123")
...
func TestToUserListResponse(t *testing.T) {
	u1, err := aggregate.NewUser("user1@example.com", "hashedPassword1")
	require.NoError(t, err)
	u2, err := aggregate.NewUser("user2@example.com", "hashedPassword2")
	require.NoError(t, err)

	responses := ToUserListResponse([]*aggregate.User{u1, u2})
