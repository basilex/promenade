...
func TestToProfileResponse(t *testing.T) {
	userID := uuidv7.New()
	p, err := aggregate.NewProfile(userID, "John Doe")
	require.NoError(t, err)

	p.Bio = "Software Engineer"
	p.Gender = aggregate.GenderMale
...
func TestToProfileResponseList(t *testing.T) {
	...
	p1, err := aggregate.NewProfile(userID1, "John Doe")
	require.NoError(t, err)
	p2, err := aggregate.NewProfile(userID2, "Jane Smith")
	require.NoError(t, err)

	responses := ToProfileResponseList([]*aggregate.Profile{p1, p2})
