...
type mockRepository struct {
	profiles         map[uuidv7.UUID]*Profile
	profilesByUser   map[uuidv7.UUID]*Profile
	createErr        error
	getErr           error
	updateErr        error
	deleteErr        error
...
func newMockRepository() *mockRepository {
	return &mockRepository{
		profiles:       make(map[uuidv7.UUID]*Profile),
		profilesByUser: make(map[uuidv7.UUID]*Profile),
	}
}
...
func (m *mockRepository) GetByID(ctx context.Context, id uuidv7.UUID) (*Profile, error) {
	if m.getErr != nil {
		return nil, m.getErr
	}
...
func (m *mockRepository) GetByUserID(ctx context.Context, userID uuidv7.UUID) (*Profile, error) {
	if m.getErr != nil {
		return nil, m.getErr
	}
...
func (m *mockRepository) ListPublicProfiles(ctx context.Context, limit, offset int) ([]*Profile, error) {
	profiles := []*Profile{}
...
func TestUseCase_UpdateGender(t *testing.T) {
	...
	err := uc.UpdateGender(ctx, profileID, GenderMale)

	require.NoError(t, err)
	assert.Equal(t, GenderMale, repo.profiles[profileID].Gender)
}
