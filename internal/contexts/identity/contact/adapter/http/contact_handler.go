...
import (
	...
	"github.com/basilex/promenade/internal/contexts/identity/contact/dto"
...
type ContactHandler struct {
	usecase usecase.IContactUseCase
}

// NewContactHandler creates a new contact handler
func NewContactHandler(uc usecase.IContactUseCase) *ContactHandler {
...
func (h *ContactHandler) GetByID(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, contacterrors.ErrContactNotFound) {
...
func (h *ContactHandler) Update(c *gin.Context) {
	...
	if req.Email != nil && existing.Type == aggregate.ContactTypeEmail {
	...
	if req.Phone != nil && existing.Type == aggregate.ContactTypePhone {
	...
	if req.Address != nil && existing.Type == aggregate.ContactTypeAddress {
...
func (h *ContactHandler) List(c *gin.Context) {
	...
	var contacts []*aggregate.Contact

	if contactType != "" {
		contacts, err = h.usecase.GetUserContactsByType(
			c.Request.Context(),
			userUUID,
			aggregate.ContactType(contactType),
		)
	} else {
