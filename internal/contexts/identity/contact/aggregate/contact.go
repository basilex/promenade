...
func NewContact(userID uuidv7.UUID, contactType ContactType, label string) (*Contact, error) {
	...
	if label == "" {
		return nil, contacterrors.ErrLabelRequired
	}
...
func (c *Contact) SetEmail(email string) error {
	if c.Type != ContactTypeEmail {
		return contacterrors.ErrCannotSetEmailOnNonEmailContact
	}
...
func (c *Contact) SetPhone(phone string) error {
	if c.Type != ContactTypePhone {
		return contacterrors.ErrCannotSetPhoneOnNonPhoneContact
	}
...
func (c *Contact) SetAddress(street, city, country, postalCode string) error {
	if c.Type != ContactTypeAddress {
		return contacterrors.ErrCannotSetAddressOnNonAddressContact
	}
...
func (c *Contact) UpdateLabel(label string) error {
	if label == "" {
		return contacterrors.ErrLabelEmpty
	}
	c.Label = label
	c.Touch()
	return nil
...
func (c *Contact) Validate() error {
	...
	// Check label
	if c.Label == "" {
		return contacterrors.ErrLabelRequired
	}
	...
	case ContactTypeEmail:
		if c.Email == nil {
			return contacterrors.ErrEmailRequiredForEmailContact
		}
		// Email is already validated in NewEmail
	case ContactTypePhone:
		if c.Phone == nil {
			return contacterrors.ErrPhoneRequiredForPhoneContact
		}
		// Phone is already validated in NewPhone
	case ContactTypeAddress:
		if c.Address == nil {
			return contacterrors.ErrAddressRequiredForAddressContact
		}
		// Address is already validated in NewAddress
	default:
		return contacterrors.ErrUnknownContactType
	}
...
func validateContactType(t ContactType) error {
	...
	case ContactTypeEmail, ContactTypePhone, ContactTypeAddress:
		return nil
	default:
		return contacterrors.ErrInvalidContactType
	}
}
