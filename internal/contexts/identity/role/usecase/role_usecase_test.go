...
type mockRoleRepository struct {
	roles        map[string]*aggregate.Role
	existsByName bool
	createErr    error
	getErr       error
	updateErr    error
...
func newMockRoleRepository() *mockRoleRepository {
	return &mockRoleRepository{
		roles: make(map[string]*aggregate.Role),
	}
}
...
func (m *mockRoleRepository) GetByID(ctx context.Context, id uuidv7.UUID) (*aggregate.Role, error) {
	if m.getErr != nil {
		return nil, m.getErr
	}
...
func (m *mockRoleRepository) GetByName(ctx context.Context, name string) (*aggregate.Role, error) {
	if m.getErr != nil {
		return nil, m.getErr
	}
...
func (m *mockRoleRepository) ListRoles(ctx context.Context, limit, offset int) ([]*aggregate.Role, int, error) {
	if m.listErr != nil {
		return nil, 0, m.listErr
	}
	roles := make([]*aggregate.Role, 0, len(m.roles))
	for _, role := range m.roles {
...
func (m *mockRoleRepository) GetUserRoles(ctx context.Context, userID uuidv7.UUID) ([]*aggregate.Role, error) {
	if m.getUserErr != nil {
...
func TestUseCase_CreateRole(t *testing.T) {
	...
	t.Run("name required", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, roleerrors.ErrRoleNameRequired)
	})
	...
	t.Run("display name required", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, roleerrors.ErrRoleDisplayRequired)
	})
	...
	t.Run("duplicate name", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, roleerrors.ErrRoleNameExists)
	})
...
func TestUseCase_GetRole(t *testing.T) {
	...
	t.Run("role not found", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, roleerrors.ErrRoleNotFound)
	})
}
...
func TestUseCase_DeleteRole(t *testing.T) {
	...
	t.Run("cannot delete system role", func(t *testing.T) {
		...
		// Create system role
		systemRole, _ := aggregate.NewSystemRole("admin", "Administrator", "System admin role")
		_ = repo.Create(ctx, systemRole)

		// Try to delete
		err := uc.DeleteRole(ctx, systemRole.ID)

		assert.ErrorIs(t, err, roleerrors.ErrCannotDeleteSystem)
	})
