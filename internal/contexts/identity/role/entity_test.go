...
func TestNewRole(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			role, err := NewRole(tt.roleName, tt.description)
...
func TestNewSystemRole(t *testing.T) {
	role, err := NewSystemRole("admin", "System administrator")
...
func TestRole_UpdateDescription(t *testing.T) {
	role, _ := NewRole("manager", "Manager role")
...
func TestRole_CanDelete(t *testing.T) {
	t.Run("system role cannot be deleted", func(t *testing.T) {
		role, _ := NewSystemRole("admin", "Admin")
		err := role.CanDelete()
		assert.Error(t, err)
	...
	t.Run("regular role can be deleted", func(t *testing.T) {
		role, _ := NewRole("custom", "Custom role")
		err := role.CanDelete()
		assert.NoError(t, err)
	})
}
