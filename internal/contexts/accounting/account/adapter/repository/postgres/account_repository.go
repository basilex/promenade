...
func (r *AccountRepository) Create(ctx context.Context, acc *aggregate.Account) error {
	...
	_, err := r.getExecutor(ctx).ExecContext(ctx, q, acc.ID.String(), acc.OrganizationID.String(), acc.Code, acc.Name, string(acc.Type), pid, acc.Level, acc.CurrencyCode, acc.IsActive, nilUUID, acc.LastUpdatedBy.String(), acc.CreatedAt, acc.UpdatedAt)
	if err != nil {
