...
import (
	...
	"github.com/basilex/promenade/internal/contexts/shared/timezone/usecase"
...
type Handler struct {
	u usecase.ITimezoneUseCase
}

// NewHandler creates a new timezone handler
func NewHandler(u usecase.ITimezoneUseCase) *Handler {
	return &Handler{u: u}
}
...
func (h *Handler) GetTimezoneByName(c *gin.Context) {
	...
	timezone, err := h.u.GetByName(ctx, name)
	if err != nil {
...
func (h *Handler) UpdateTimezone(c *gin.Context) {
	...
	response.Success(c, dto.ToTimezoneResponse(existing))
}
