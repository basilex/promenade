...
func NewCustomer(name, email, source string, assignedTo uuidv7.UUID) (*Customer, error) {
	if name == "" {
		return nil, customererrors.ErrCustomerNameEmpty
	}

	emailVO, err := valueobject.NewEmail(email)
	if err != nil {
		return nil, customererrors.ErrCustomerEmailInvalid
	}

	if source == "" {
		return nil, customererrors.ErrCustomerSourceEmpty
	}

	if assignedTo == uuidv7.Nil {
		return nil, customererrors.ErrCustomerSalesRepEmpty
	}
...
func NewB2BCustomer(name, email, source string, companyID, assignedTo uuidv7.UUID) (*Customer, error) {
	if companyID == uuidv7.Nil {
		return nil, customererrors.ErrCustomerCompanyIDEmpty
	}
...
func (c *Customer) SetPhone(phone string) error {
	...
	if err != nil {
		return customererrors.ErrCustomerPhoneInvalid
	}
...
func (c *Customer) LinkToUser(userID uuidv7.UUID) error {
	if userID == uuidv7.Nil {
		return customererrors.ErrCustomerUserIDEmpty
	}

	if c.UserID != nil {
		return customererrors.ErrCustomerUserAlreadyLinked
	}
...
func (c *Customer) QualifyAsProspect() error {
	if c.Status != CustomerStatusLead {
		return customererrors.ErrInvalidStatusTransition
	}
...
func (c *Customer) Churn(reason string) error {
	...
	if reason == "" {
		return customererrors.ErrCustomerChurnReasonEmpty
	}
...
func (c *Customer) Reassign(newRepID uuidv7.UUID) error {
	if newRepID == uuidv7.Nil {
		return customererrors.ErrCustomerSalesRepEmpty
	}

	if c.AssignedTo == newRepID {
		return customererrors.ErrCustomerAssignedToSameRep
	}
...
func (c *Customer) UpgradeTier(newTier CustomerTier) error {
	if !isValidTier(newTier) {
		return customererrors.ErrInvalidTierTransition
	}
...
func (c *Customer) AddTag(tag string) error {
	if tag == "" {
		return customererrors.ErrCustomerTagEmpty
	}

	for _, t := range c.Tags {
		if t == tag {
			return customererrors.ErrCustomerTagAlreadyExists
		}
	}
...
func (c *Customer) RemoveTag(tag string) error {
	...
	if !found {
		return customererrors.ErrCustomerTagNotFound
	}
...
func (c *Customer) Validate() error {
	if c.Name == "" {
		return customererrors.ErrCustomerNameEmpty
	}

	if c.Email.Value() == "" {
		return customererrors.ErrCustomerEmailInvalid
	}
	...
	if c.Source == "" {
		return customererrors.ErrCustomerSourceEmpty
	}
