...
func NewCustomerHandler(uc usecase.ICustomerUseCase) *CustomerHandler {
...
func (h *CustomerHandler) Create(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, customererrors.ErrCustomerNameEmpty) ||
			errors.Is(err, customererrors.ErrCustomerEmailInvalid) ||
			errors.Is(err, customererrors.ErrCustomerSourceEmpty) ||
			errors.Is(err, customererrors.ErrCustomerSalesRepEmpty) {
			response.BadRequest(c, err.Error())
			return
		}
		// Already exists → 409
		if errors.Is(err, customererrors.ErrCustomerAlreadyExists) {
...
func (h *CustomerHandler) CreateB2B(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, customererrors.ErrCustomerNameEmpty) ||
			...
			errors.Is(err, customererrors.ErrCustomerSourceEmpty) ||
			errors.Is(err, customererrors.ErrCustomerSalesRepEmpty) ||
			errors.Is(err, customererrors.ErrCustomerCompanyIDEmpty) {
			response.BadRequest(c, err.Error())
			return
		}
		// Already exists → 409
	...
	if req.Phone != "" {
		if err := h.usecase.SetCustomerPhone(c.Request.Context(), created.ID, req.Phone); err != nil {
			if errors.Is(err, customererrors.ErrCustomerPhoneInvalid) {
				response.BadRequest(c, err.Error())
				return
			}
			response.InternalError(c, "Failed to set customer phone")
	...
	for _, tag := range req.Tags {
		if err := h.usecase.AddTagToCustomer(c.Request.Context(), created.ID, tag); err != nil {
			if errors.Is(err, customererrors.ErrCustomerNotFound) {
...
func (h *CustomerHandler) GetByID(c *gin.Context) {
	...
	response.Success(c, dto.ToCustomerResponse(cust))
}
...
func (h *CustomerHandler) List(c *gin.Context) {
	...
	response.Success(c, gin.H{
		"customers": dto.ToCustomerListResponse(customers),
		"total":     total,
		"limit":     limit,
		"offset":    offset,
	})
...
func (h *CustomerHandler) QualifyAsProspect(c *gin.Context) {
	...
	if err := h.usecase.QualifyAsProspect(c.Request.Context(), customerID); err != nil {
		...
		if errors.Is(err, customererrors.ErrInvalidStatusTransition) {
...
func (h *CustomerHandler) Churn(c *gin.Context) {
	...
	var req dto.ChurnCustomerRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
...
func (h *CustomerHandler) UpgradeTier(c *gin.Context) {
	...
	if err := h.usecase.UpgradeCustomerTier(c.Request.Context(), customerID, tier); err != nil {
		...
		if errors.Is(err, customererrors.ErrInvalidTierTransition) {
			response.Conflict(c, err.Error())
			return
		}
...
func (h *CustomerHandler) ListByStatus(c *gin.Context) {
	...
	status := aggregate.CustomerStatus(statusStr)
...
func (h *CustomerHandler) GetStats(c *gin.Context) {
	...
	response.Success(c, dto.ToCustomerStatsResponse(stats))
}
...
func (h *CustomerHandler) AssignTo(c *gin.Context) {
	var req dto.ReassignCustomerRequest
