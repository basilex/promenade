...
func (m *MockRepository) Create(ctx context.Context, interaction *Interaction) error {
	args := m.Called(ctx, interaction)
	return args.Error(0)
}

func (m *MockRepository) GetByID(ctx context.Context, id uuidv7.UUID) (*Interaction, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*Interaction), args.Error(1)
}

func (m *MockRepository) Update(ctx context.Context, interaction *Interaction) error {
	args := m.Called(ctx, interaction)
	return args.Error(0)
}
...
func (m *MockRepository) ListByCustomer(ctx context.Context, customerID uuidv7.UUID, page, pageSize int) ([]*Interaction, int64, error) {
	args := m.Called(ctx, customerID, page, pageSize)
	if args.Get(0) == nil {
		return nil, args.Get(1).(int64), args.Error(2)
	}
	return args.Get(0).([]*Interaction), args.Get(1).(int64), args.Error(2)
}

func (m *MockRepository) ListByCompany(ctx context.Context, companyID uuidv7.UUID, page, pageSize int) ([]*Interaction, int64, error) {
...
func (m *MockRepository) ListByType(ctx context.Context, interactionType string, page, pageSize int) ([]*Interaction, int64, error) {
...
func (m *MockRepository) ListByCreatedBy(ctx context.Context, createdBy uuidv7.UUID, page, pageSize int) ([]*Interaction, int64, error) {
...
func (m *MockRepository) ListPendingFollowUps(ctx context.Context, page, pageSize int) ([]*Interaction, int64, error) {
...
// Test Helpers
func createTestUseCase() (*useCase, *MockRepository) {
	mockRepo := new(MockRepository)
	uc := NewUseCase(mockRepo).(*useCase)
	return uc, mockRepo
}
...
func TestUseCase_CreateInteraction(t *testing.T) {
	...
	t.Run("successful creation", func(t *testing.T) {
		...
		mockRepo.On("Create", ctx, mock.AnythingOfType("*interaction.Interaction")).
			Return(nil)
		...
		interaction, err := uc.CreateInteraction(
			...
			customerID,
			nil,
			string(InteractionTypeCall),
			string(InteractionDirectionOutbound),
			"Test call",
			"Test description",
			createdBy,
			startedAt,
		...
		assert.Equal(t, InteractionTypeCall, interaction.Type)
	...
	t.Run("with company ID", func(t *testing.T) {
		...
		interaction, err := uc.CreateInteraction(
			...
			customerID,
			&companyID,
			string(InteractionTypeMeeting),
			string(InteractionDirectionInbound),
			"Client meeting",
...
func TestUseCase_SetOutcome(t *testing.T) {
	...
	t.Run("successful outcome set", func(t *testing.T) {
		...
		updated, err := uc.SetOutcome(ctx, interactionID, string(InteractionOutcomeSuccessful))

		require.NoError(t, err)
		assert.NotNil(t, updated)
		assert.Equal(t, InteractionOutcomeSuccessful, *updated.Outcome)
		mockRepo.AssertExpectations(t)
	})
}
...
func TestUseCase_ListByType(t *testing.T) {
	...
	t.Run("valid type", func(t *testing.T) {
		...
		interactions, total, err := uc.ListByType(ctx, string(InteractionTypeCall), 1, 20)
	...
	t.Run("invalid type", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.ErrorIs(t, err, ErrInvalidInteractionType)
	})
}
