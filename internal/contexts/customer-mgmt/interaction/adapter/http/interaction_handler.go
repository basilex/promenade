...
type InteractionHandler struct {
	usecase usecase.IInteractionUseCase
}

// NewInteractionHandler creates a new interaction handler
func NewInteractionHandler(usecase usecase.IInteractionUseCase) *InteractionHandler {
...
// @Accept json
// @Produce json
// @Param request body dto.CreateInteractionRequest true "Interaction data"
// @Success 201 {object} response.Response{data=dto.InteractionResponse}
...
func (h *InteractionHandler) Create(c *gin.Context) {
	...
	// Parse UUIDs
	customerID, err := parseInteractionUUID(req.CustomerID)
	if err != nil {
	...
	if req.CompanyID != nil && *req.CompanyID != "" {
		id, err := parseInteractionUUID(*req.CompanyID)
		if err != nil {
	...
	createdBy, err := parseInteractionUUID(req.CreatedBy)
	if err != nil {
		response.BadRequest(c, "invalid created_by")
		return
	}

	startedAt, err := parseInteractionTime(req.StartedAt)
	if err != nil {
	...
	response.Created(c, dto.ToInteractionResponse(inter))
}
...
// @Success 200 {object} response.Response{data=[]dto.InteractionResponse}
// @Failure 500 {object} response.Response
...
func (h *InteractionHandler) List(c *gin.Context) {
	...
	respData := make([]*dto.InteractionResponse, len(interactions))
	for i, inter := range interactions {
		respData[i] = dto.ToInteractionResponse(inter)
	}
...
func (h *InteractionHandler) GetByID(c *gin.Context) {
	id, err := parseInteractionUUID(c.Param("id"))
	if err != nil {
	...
	if err != nil {
		if err == interactionerrors.ErrInteractionNotFound {
	...
	response.Success(c, dto.ToInteractionResponse(inter))
}
...
func (h *InteractionHandler) ListByCustomer(c *gin.Context) {
	customerID, err := parseInteractionUUID(c.Param("customer_id"))
	if err != nil {
		response.BadRequest(c, "invalid customer_id")
		return
	}
...
func (h *InteractionHandler) ListByCompany(c *gin.Context) {
	companyID, err := parseInteractionUUID(c.Param("company_id"))
	if err != nil {
		response.BadRequest(c, "invalid company_id")
		return
	}
...
// @Produce json
// @Param id path string true "Interaction ID"
// @Param request body dto.UpdateContentRequest true "Content data"
// @Success 200 {object} response.Response{data=dto.InteractionResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
// @Failure 500 {object} response.Response
...
// @Produce json
// @Param id path string true "Interaction ID"
// @Param request body dto.SetOutcomeRequest true "Outcome data"
// @Success 200 {object} response.Response{data=dto.InteractionResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
// @Failure 500 {object} response.Response
...
// @Produce json
// @Param id path string true "Interaction ID"
// @Param request body dto.EndInteractionRequest true "End time data"
// @Success 200 {object} response.Response{data=dto.InteractionResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
// @Failure 500 {object} response.Response
...
func (h *InteractionHandler) EndInteraction(c *gin.Context) {
	...
	var req dto.EndInteractionRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, err.Error())
		return
	}

	endedAt, err := parseInteractionTime(req.EndedAt)
	if err != nil {
	...
	if err != nil {
		...
		if err == interactionerrors.ErrInteractionAlreadyEnded {
...
// @Produce json
// @Param id path string true "Interaction ID"
// @Param request body dto.SetFollowUpRequest true "Follow-up data"
// @Success 200 {object} response.Response{data=dto.InteractionResponse}
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
// @Failure 500 {object} response.Response
...
func (h *InteractionHandler) SetFollowUp(c *gin.Context) {
	...
	var req dto.SetFollowUpRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
...
func (h *InteractionHandler) AddAttendee(c *gin.Context) {
	...
	attendeeID, err := parseInteractionUUID(req.AttendeeID)
	if err != nil {
...
func (h *InteractionHandler) RemoveAttendee(c *gin.Context) {
	...
	attendeeID, err := parseInteractionUUID(c.Param("attendee_id"))
	if err != nil {
		response.BadRequest(c, "invalid attendee_id")
		return
	}
