...
import (
	...
	deal "github.com/basilex/promenade/internal/contexts/customer-mgmt/deal"
...
func NewDeal(customerID uuidv7.UUID, name string, value valueobject.Money, assignedTo uuidv7.UUID, expectedCloseDate time.Time) (*Deal, error) {
	if name == "" {
		return nil, deal.ErrDealNameEmpty
	}
	if customerID == uuidv7.Nil {
		return nil, deal.ErrDealCustomerRequired
	}
	if assignedTo == uuidv7.Nil {
		return nil, deal.ErrDealAssignedToRequired
	}
	if value.Amount < 0 {
		return nil, deal.ErrDealValueNegative
	}
	if expectedCloseDate.Before(time.Now()) {
		return nil, deal.ErrDealDateInPast
	}
...
func (d *Deal) UpdateValue(value valueobject.Money) error {
	if value.Amount < 0 {
		return deal.ErrDealValueNegative
	}
	d.Value = value
...
func (d *Deal) MoveTo(stage DealStage) error {
	if d.Stage == DealStageClosedWon || d.Stage == DealStageClosedLost {
		return deal.ErrDealTerminalStage
	}

	if !isValidStageTransition(d.Stage, stage) {
		return deal.ErrDealInvalidStageTransition
	}
...
func (d *Deal) MarkAsWon(reason string) error {
	if d.Stage == DealStageClosedWon {
		return deal.ErrDealAlreadyWon
	}
	if d.Stage == DealStageClosedLost {
		return deal.ErrDealCannotMarkLostAsWon
	}
...
func (d *Deal) MarkAsLost(reason string) error {
	if d.Stage == DealStageClosedLost {
		return deal.ErrDealAlreadyLost
	}
	if d.Stage == DealStageClosedWon {
		return deal.ErrDealCannotMarkWonAsLost
	}
	if reason == "" {
		return deal.ErrDealLossReasonRequired
	}
...
func (d *Deal) UpdateProbability(probability int) error {
	if probability < 0 || probability > 100 {
		return deal.ErrDealProbabilityRange
	}
	if d.Stage == DealStageClosedWon || d.Stage == DealStageClosedLost {
		return deal.ErrDealClosedMutation
	}
...
func (d *Deal) AssignToSalesRep(userID uuidv7.UUID) error {
	if userID == uuidv7.Nil {
		return deal.ErrDealSalesRepRequired
	}
	d.AssignedTo = userID
	d.Touch()
	return nil
