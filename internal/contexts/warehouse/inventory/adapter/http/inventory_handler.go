...
func NewInventoryHandler(uc usecase.IInventoryUseCase) *InventoryHandler {
...
func (h *InventoryHandler) Create(c *gin.Context) {
	...
	if err != nil {
		...
		switch {
		case errors.Is(err, inventoryerrors.ErrInventorySKURequired):
			response.BadRequest(c, "SKU is required")
		case errors.Is(err, inventoryerrors.ErrInventoryProductNameRequired):
			response.BadRequest(c, "Product name is required")
		case errors.Is(err, inventoryerrors.ErrInventoryWarehouseRequired):
			response.BadRequest(c, "Warehouse ID is required")
		case errors.Is(err, inventoryerrors.ErrInventoryCreatedByRequired):
			response.BadRequest(c, "Created by user ID is required")
		case errors.Is(err, inventoryerrors.ErrInventorySKUExists):
	...
	response.Created(c, dto.ToInventoryResponse(inv))
}
...
func (h *InventoryHandler) GetByID(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, inventoryerrors.ErrInventoryNotFound) {
			response.NotFound(c, "aggregate.Inventory not found")
			return
		}
	...
	response.Success(c, dto.ToInventoryResponse(inv))
}
...
func (h *InventoryHandler) GetByProductID(c *gin.Context) {
	...
	response.Success(c, dto.ToInventoryResponseList(items))
}
...
func (h *InventoryHandler) List(c *gin.Context) {
	...
	response.SuccessWithPagination(c, dto.ToInventoryResponseList(items), total, page, pageSize)
}
...
func (h *InventoryHandler) Update(c *gin.Context) {
	...
	if err := h.usecase.UpdateInventory(c.Request.Context(), inv); err != nil {
		// Map domain errors to user-friendly messages
		switch {
		case errors.Is(err, inventoryerrors.ErrInventoryNotFound):
			response.NotFound(c, "aggregate.Inventory not found")
		case errors.Is(err, inventoryerrors.ErrInventoryNil):
...
func (h *InventoryHandler) Delete(c *gin.Context) {
	...
	response.Success(c, gin.H{"message": "aggregate.Inventory deleted successfully"})
}
...
func (h *InventoryHandler) ReceiveStock(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, inventoryerrors.ErrInventoryNotFound):
			response.NotFound(c, "aggregate.Inventory not found")
		case errors.Is(err, inventoryerrors.ErrInventoryQuantityInvalid):
			response.BadRequest(c, "Quantity must be greater than 0")
		case errors.Is(err, inventoryerrors.ErrInventoryUnitCostNegative):
...
func (h *InventoryHandler) CommitStock(c *gin.Context) {
	...
	var req dto.CommitStockRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	...
	if err != nil {
		...
		case errors.Is(err, inventoryerrors.ErrInventoryQuantityInvalid):
			response.BadRequest(c, "Quantity must be greater than 0")
		case errors.Is(err, inventoryerrors.ErrInventoryInsufficientStock):
...
func (h *InventoryHandler) ReserveStock(c *gin.Context) {
	...
	if err := h.usecase.UpdateInventory(c.Request.Context(), inv); err != nil {
		switch {
		case errors.Is(err, inventoryerrors.ErrVersionConflict):
			response.Conflict(c, "aggregate.Inventory was modified by another process, please retry")
		default:
			response.InternalError(c, "Failed to update inventory")
		}
		return
