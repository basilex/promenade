...
//	if errors.Is(err, aggregate.ErrInventoryNotFound) {
//	    // Handle not found
//	}
type IInventoryRepository interface {
	...
	//
	// Example:
	//   inv, _ := aggregate.NewInventory(productID, "SKU-001", "Product Name", "WH-01")
	//   err := repo.Create(ctx, inv)
	Create(ctx context.Context, inventory *aggregate.Inventory) error
	...
	//   }
	GetByID(ctx context.Context, id uuidv7.UUID) (*aggregate.Inventory, error)
	...
	//   }
	Update(ctx context.Context, inventory *aggregate.Inventory) error
	...
	List(ctx context.Context, limit, offset int) ([]*aggregate.Inventory, int64, error)
	...
	//
	// Example:
	//   damaged, err := repo.GetByStatus(ctx, aggregate.StatusDamaged)
	GetByStatus(ctx context.Context, status aggregate.InventoryStatus) ([]*aggregate.Inventory, error)
	...
	//   updates := []*aggregate.Inventory{inv1, inv2, inv3}
	//   err := repo.BulkUpdate(ctx, updates)
	BulkUpdate(ctx context.Context, inventories []*aggregate.Inventory) error
}
