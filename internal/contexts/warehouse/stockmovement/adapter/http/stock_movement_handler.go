...
import (
	...
	"github.com/basilex/promenade/internal/contexts/warehouse/stockmovement/dto"
	"github.com/basilex/promenade/internal/contexts/warehouse/stockmovement/usecase"
...
type StockMovementHandler struct {
	usecase usecase.IStockMovementUseCase
}

// NewStockMovementHandler creates a new stock movement handler
func NewStockMovementHandler(uc usecase.IStockMovementUseCase) *StockMovementHandler {
...
// @Accept json
// @Produce json
// @Param movement body dto.RecordMovementRequest true "Stock movement details"
// @Success 201 {object} response.Response{data=dto.MovementResponse} "Movement recorded successfully"
...
func (h *StockMovementHandler) RecordMovement(c *gin.Context) {
	...
	movementType := aggregate.MovementType(req.Type)
	...
	if err != nil {
		switch {
		case errors.Is(err, stockmovementerrors.ErrStockMovementNil):
			response.BadRequest(c, "Stock movement cannot be nil")
		case errors.Is(err, stockmovementerrors.ErrStockMovementValidationFailed):
			response.BadRequest(c, "Stock movement validation failed")
		case errors.Is(err, stockmovementerrors.ErrStockMovementCreateFailed):
			response.InternalError(c, "Failed to create stock movement")
		case errors.Is(err, stockmovementerrors.ErrStockMovementSaveFailed):
	...
	response.Created(c, dto.ToMovementResponse(movement))
}
...
// @Success 200 {object} response.Response{data=dto.MovementResponse} "Movement details"
...
func (h *StockMovementHandler) GetByID(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, stockmovementerrors.ErrStockMovementNotFound):
			response.NotFound(c, "Stock movement not found")
		case errors.Is(err, stockmovementerrors.ErrStockMovementGetFailed):
	...
	response.Success(c, dto.ToMovementResponse(movement))
}
...
// @Success 200 {object} response.Response{data=object{movements=[]dto.MovementResponse,total=int,page=int,page_size=int}} "Paginated movements"
...
func (h *StockMovementHandler) GetByInventory(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, stockmovementerrors.ErrInvalidPagination):
			response.BadRequest(c, "Invalid pagination parameters")
		case errors.Is(err, stockmovementerrors.ErrStockMovementListFailed):
	...
	result := map[string]interface{}{
		"movements": dto.ToMovementResponseList(movements),
		"total":     total,
		"page":      page,
		"page_size": pageSize,
	}
...
// @Success 200 {object} response.Response{data=[]dto.MovementResponse} "Movements for reference"
...
func (h *StockMovementHandler) GetByReference(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, stockmovementerrors.ErrReferenceTypeRequired):
			response.BadRequest(c, "Reference type is required")
		case errors.Is(err, stockmovementerrors.ErrReferenceIDRequired):
			response.BadRequest(c, "Reference ID is required")
		case errors.Is(err, stockmovementerrors.ErrStockMovementListFailed):
	...
	response.Success(c, dto.ToMovementResponseList(movements))
}
...
func (h *StockMovementHandler) GetByType(c *gin.Context) {
	movementType := aggregate.MovementType(c.Param("type"))
...
// @Tags Stock Movements
// @Produce json
// @Success 200 {object} response.Response{data=[]dto.MovementResponse} "Recent movements"
// @Failure 500 {object} response.Response "Internal server error"
...
func (h *StockMovementHandler) GetInventorySummary(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, stockmovementerrors.ErrStockMovementSummaryFailed):
