...
// stockMovementRepository implements repository.IStockMovementRepository using PostgreSQL.
type stockMovementRepository struct {
	db *sqlx.DB
}

// NewStockMovementRepository creates a new PostgreSQL stock movement repository.
func NewStockMovementRepository(db *sqlx.DB) repository.IStockMovementRepository {
...
func (r *stockMovementRepository) getExecutor(ctx context.Context) sqlx.ExtContext {
...
func (r *stockMovementRepository) Get(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
	return sqlx.GetContext(ctx, r.getExecutor(ctx), dest, query, args...)
}

// Select executes query and scans multiple rows
func (r *stockMovementRepository) Select(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
	return sqlx.SelectContext(ctx, r.getExecutor(ctx), dest, query, args...)
}

// Exec executes a query without returning rows
func (r *stockMovementRepository) Exec(ctx context.Context, query string, args ...interface{}) (sql.Result, error) {
	return r.getExecutor(ctx).ExecContext(ctx, query, args...)
}

// NamedExec executes a named query without returning rows
func (r *stockMovementRepository) NamedExec(ctx context.Context, query string, arg interface{}) (sql.Result, error) {
...
func (r *stockMovementRow) toEntity() (*aggregate.StockMovement, error) {
	...
	sm := &aggregate.StockMovement{}
	sm.ID = id
	sm.InventoryID = inventoryID
	sm.Type = aggregate.MovementType(r.Type)
...
func fromEntity(sm *aggregate.StockMovement) *stockMovementRow {
...
func (r *stockMovementRepository) Create(ctx context.Context, movement *aggregate.StockMovement) error {
	if movement == nil {
...
func (r *stockMovementRepository) GetByID(ctx context.Context, id uuidv7.UUID) (*aggregate.StockMovement, error) {
...
func (r *stockMovementRepository) GetByInventoryID(ctx context.Context, inventoryID uuidv7.UUID, page, pageSize int) ([]*aggregate.StockMovement, int, error) {
	if page < 1 {
		page = 1
	}
	...
	movements := make([]*aggregate.StockMovement, 0, len(rows))
...
func (r *stockMovementRepository) GetByReference(ctx context.Context, referenceType string, referenceID uuidv7.UUID) ([]*aggregate.StockMovement, error) {
	var rows []stockMovementRow
	query := `
		SELECT id, inventory_id, type, quantity, quantity_before_move, quantity_after_move,
		       from_warehouse_id, from_location_code, to_warehouse_id, to_location_code,
...
func (r *stockMovementRepository) GetByType(ctx context.Context, movementType aggregate.MovementType, startDate, endDate time.Time, page, pageSize int) ([]*aggregate.StockMovement, int, error) {
...
func (r *stockMovementRepository) GetByDateRange(ctx context.Context, startDate, endDate time.Time, page, pageSize int) ([]*aggregate.StockMovement, int, error) {
	if startDate.After(endDate) {
		return nil, 0, stockmovementerrors.ErrInvalidDateRange
	}
...
func (r *stockMovementRepository) GetRecentMovements(ctx context.Context, limit int) ([]*aggregate.StockMovement, error) {
...
func (r *stockMovementRepository) CountByType(ctx context.Context, startDate, endDate time.Time) (map[aggregate.MovementType]int, error) {
	if startDate.After(endDate) {
		return nil, stockmovementerrors.ErrInvalidDateRange
	}
	...
	counts := make(map[aggregate.MovementType]int)
	for _, result := range results {
		counts[aggregate.MovementType(result.Type)] = result.Count
	}
