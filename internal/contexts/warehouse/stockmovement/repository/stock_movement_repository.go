...
// IStockMovementRepository defines persistence operations for StockMovement aggregate.
//
// Design Principles:
...
type IStockMovementRepository interface {
	...
	Create(ctx context.Context, movement *aggregate.StockMovement) error
	...
	//   }
	GetByID(ctx context.Context, id uuidv7.UUID) (*aggregate.StockMovement, error)
	...
	GetByInventoryID(ctx context.Context, inventoryID uuidv7.UUID, page, pageSize int) ([]*aggregate.StockMovement, int, error)
	...
	GetByReference(ctx context.Context, referenceType string, referenceID uuidv7.UUID) ([]*aggregate.StockMovement, error)
	...
	GetByType(ctx context.Context, movementType aggregate.MovementType, startDate, endDate time.Time, page, pageSize int) ([]*aggregate.StockMovement, int, error)
	...
	GetByDateRange(ctx context.Context, startDate, endDate time.Time, page, pageSize int) ([]*aggregate.StockMovement, int, error)
	...
	// Returns map of aggregate.MovementType -> count.
	//
	// Example:
	//   counts, err := repo.CountByType(ctx, startDate, endDate)
	//   // Returns: {receipt: 100, commit: 80, adjustment: 5, ...}
	CountByType(ctx context.Context, startDate, endDate time.Time) (map[aggregate.MovementType]int, error)
}
