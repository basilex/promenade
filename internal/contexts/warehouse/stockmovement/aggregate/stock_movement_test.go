...
func TestNewStockMovement_ZeroQuantity(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Nil(t, sm)
	assert.ErrorIs(t, err, stockmovementerrors.ErrQuantityCannotBeZero)
}
...
func TestNewStockMovement_InvalidMovementType(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Nil(t, sm)
	assert.ErrorIs(t, err, stockmovementerrors.ErrInvalidMovementType)
}
...
func TestValidate_AdjustmentWithoutReason(t *testing.T) {
	...
	err := sm.Validate()
	assert.Error(t, err)
	assert.ErrorIs(t, err, stockmovementerrors.ErrAdjustmentRequiresReason)
}
