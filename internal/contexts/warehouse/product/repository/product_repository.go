...
// IProductRepository defines the contract for Product persistence operations.
//
// Implementation notes:
// - All methods accept context for transaction support
// - GetBy* methods return producterrors.ErrProductNotFound if not found
...
type IProductRepository interface {
	...
	Create(ctx context.Context, product *aggregate.Product) error

	// GetByID retrieves a product by its UUID.
	// Returns ErrProductNotFound if product doesn't exist or is soft-deleted.
	GetByID(ctx context.Context, id uuidv7.UUID) (*aggregate.Product, error)

	// GetBySKU retrieves a product by its SKU (unique identifier).
	// Returns ErrProductNotFound if product doesn't exist or is soft-deleted.
	GetBySKU(ctx context.Context, sku string) (*aggregate.Product, error)
	...
	List(ctx context.Context, page, pageSize int) ([]*aggregate.Product, error)

	// ListByCategory retrieves products by category with pagination.
	ListByCategory(ctx context.Context, category string, page, pageSize int) ([]*aggregate.Product, error)

	// ListByBrand retrieves products by brand with pagination.
	ListByBrand(ctx context.Context, brand string, page, pageSize int) ([]*aggregate.Product, error)

	// ListByStatus retrieves products by status with pagination.
	ListByStatus(ctx context.Context, status aggregate.ProductStatus, page, pageSize int) ([]*aggregate.Product, error)

	// Search performs full-text search on product name, description, SKU.
	// Returns products matching search query with pagination.
	Search(ctx context.Context, query string, page, pageSize int) ([]*aggregate.Product, error)
	...
	GetByIDs(ctx context.Context, ids []uuidv7.UUID) ([]*aggregate.Product, error)
	...
	ListLowStock(ctx context.Context, page, pageSize int) ([]*aggregate.Product, error)
}
