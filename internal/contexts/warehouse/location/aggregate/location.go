...
func NewLocation(code, name string, locType LocationType) (*Location, error) {
	if code == "" {
		return nil, locationerrors.ErrLocationCodeRequired
	}
	if name == "" {
		return nil, locationerrors.ErrLocationNameRequired
	}
	if !isValidLocationType(locType) {
		return nil, locationerrors.ErrInvalidLocationType
	}
...
func (l *Location) SetParent(parentID uuidv7.UUID, parentPath string, parentLevel int) error {
	if parentID == l.GetID() {
		return locationerrors.ErrCannotBeOwnParent
	}
...
func (l *Location) SetCapacity(capacity int, enforce bool) error {
	if capacity < 0 {
		return locationerrors.ErrNegativeCapacity
	}
	if capacity < l.CurrentOccupancy {
		return locationerrors.ErrCapacityLessThanOccupancy
	}
...
func (l *Location) SetDimensions(width, height, depth float64) error {
	if width < 0 || height < 0 || depth < 0 {
		return locationerrors.ErrNegativeDimensions
	}
...
func (l *Location) AddOccupancy(amount int) error {
	if amount <= 0 {
		return locationerrors.ErrAmountMustBePositive
	}

	newOccupancy := l.CurrentOccupancy + amount

	if l.IsLimited && newOccupancy > l.Capacity {
		return locationerrors.ErrInsufficientCapacity
	}
...
func (l *Location) RemoveOccupancy(amount int) error {
	...
	if amount > l.CurrentOccupancy {
		return locationerrors.ErrCannotRemoveExcessOccupancy
	}
...
func (l *Location) Activate() error {
	if l.Status == LocationStatusActive {
		return locationerrors.ErrLocationAlreadyActive
	}
...
func (l *Location) Deactivate() error {
	if l.Status == LocationStatusInactive {
		return locationerrors.ErrLocationAlreadyInactive
	}
	if l.CurrentOccupancy > 0 {
		return locationerrors.ErrCannotDeactivateWithItems
	}
...
func (l *Location) SetMaintenance() error {
	if l.Status == LocationStatusMaintenance {
		return locationerrors.ErrLocationAlreadyInMaintenance
	}
...
func (l *Location) Validate() error {
	if l.Code == "" {
		return locationerrors.ErrLocationCodeRequired
	}
	if l.Name == "" {
		return locationerrors.ErrLocationNameRequired
	}
	if !isValidLocationType(l.Type) {
		return locationerrors.ErrInvalidLocationType
	}
	if !isValidLocationStatus(l.Status) {
		return locationerrors.ErrInvalidLocationStatus
	}
	if l.CurrentOccupancy < 0 {
		return locationerrors.ErrNegativeOccupancy
	}
	if l.IsLimited && l.Capacity < 0 {
		return locationerrors.ErrNegativeCapacity
	}
	if l.IsLimited && l.CurrentOccupancy > l.Capacity {
		return locationerrors.ErrOccupancyExceedsCapacity
	}
