...
func TestNewOrder(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			currency:    "USD",
			wantErr:     true,
			expectedErr: ordererrors.ErrCustomerIDRequired,
		},
		{
			name:        "empty currency",
			customerID:  uuidv7.New(),
			currency:    "",
			wantErr:     true,
			expectedErr: ordererrors.ErrCurrencyRequired,
		},
	}
...
func TestOrder_AddLine(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			currency:    "USD",
			wantErr:     true,
			expectedErr: ordererrors.ErrInvalidQuantity,
		},
		{
		...
		},
		{
			...
			currency:    "USD",
			wantErr:     true,
			expectedErr: ordererrors.ErrInvalidPrice,
		},
		{
			...
			currency:    "EUR",
			wantErr:     true,
			expectedErr: ordererrors.ErrCurrencyMismatch,
		},
	}
...
func TestOrder_RemoveLine(t *testing.T) {
	...
	require.Error(t, err)
	assert.ErrorIs(t, err, ordererrors.ErrOrderLineNotFound)
}
...
func TestOrder_Confirm(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			addLines:    false,
			wantErr:     true,
			expectedErr: ordererrors.ErrOrderEmpty,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = order.Confirm()
			require.Error(t, err)
			assert.ErrorIs(t, err, ordererrors.ErrInvalidOrderStatus)
		})
	}
}
...
func TestOrder_Cancel(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr:       true,
			expectedErr:   ordererrors.ErrCannotCancelFulfilled,
		},
	}
