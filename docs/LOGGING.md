...
Promenade uses **slog** (structured logging from Go 1.21+) for high-performance, machine-parseable logging.
...
Automatically logs all HTTP requests with structured fields:
...
Logs panics with full stack trace:
...
JSON format is easily parsed by systems:
...
- Zero allocation for basic types
