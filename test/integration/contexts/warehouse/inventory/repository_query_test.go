...
func createTestInventoryForProduct(productID uuidv7.UUID, warehouseID, locationCode string, quantity int) *inventory.Inventory {
...
func TestInventoryRepository_GetByStatus(t *testing.T) {
	...
	inv1.SetStatus(inventory.InventoryStatusAvailable)
	inv1.LastUpdatedBy = userID
	require.NoError(t, repo.Create(ctx, inv1))

	inv2 := createTestInventory()
	inv2.SetStatus(inventory.InventoryStatusDamaged)
	inv2.LastUpdatedBy = userID
	require.NoError(t, repo.Create(ctx, inv2))

	inv3 := createTestInventory()
	inv3.SetStatus(inventory.InventoryStatusQuarantined)
	inv3.LastUpdatedBy = userID
	require.NoError(t, repo.Create(ctx, inv3))

		// Test: Get damaged items only
		damagedItems, err := repo.GetByStatus(ctx, inventory.InventoryStatusDamaged)
		require.NoError(t, err)
		assert.GreaterOrEqual(t, len(damagedItems), 1)
		for _, item := range damagedItems {
			assert.Equal(t, inventory.InventoryStatusDamaged, item.Status)
		}
	})
}
...
func TestInventoryRepository_BulkUpdate(t *testing.T) {
	...
	userID := uuidv7.New()
	items := make([]*inventory.Inventory, 3)
	for i := 0; i < 3; i++ {
