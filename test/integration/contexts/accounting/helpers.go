...
// CreateTestGLAccount creates a test GL account in the chart of accounts
func CreateTestGLAccount(ctx context.Context, tx *sqlx.Tx, orgID, userID uuidv7.UUID, code, name, accountType string) (uuidv7.UUID, error) {
	...
	query := `
        INSERT INTO accounting_chart_of_accounts (
            id, version, organization_id, code, name, account_type,
            level, is_active, currency_code, last_updated_by,
