...
func createTestOrder(ctx context.Context, tx *sqlx.Tx, customerID uuidv7.UUID) (uuidv7.UUID, error) {
	orderID := uuidv7.New()
	// Use unique order number with timestamp and random suffix to avoid conflicts
	orderNumber := fmt.Sprintf("ORD-%d-%04d", time.Now().Unix(), rand.Intn(10000))
	_, err := tx.ExecContext(ctx, `
		INSERT INTO order_orders (id, order_number, customer_id, total_amount, currency, status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
	`, orderID, orderNumber, customerID, 0.0, "USD", "pending")
	return orderID, err
}
...
func TestContractRepository_GetByOrder(t *testing.T) {
	...
	testDB.WithTransaction(t, func(ctx context.Context, tx *sqlx.Tx) {
		...
		customerID, err := createTestCustomer(ctx, tx)
		require.NoError(t, err)
		orderID1, err := createTestOrder(ctx, tx, customerID)
		require.NoError(t, err)
		orderID2, err := createTestOrder(ctx, tx, customerID)
		require.NoError(t, err)
...
func TestContractRepository_GetByCustomer(t *testing.T) {
	...
	testDB.WithTransaction(t, func(ctx context.Context, tx *sqlx.Tx) {
		...
		// Create dependencies
		customerID1, err := createTestCustomer(ctx, tx)
		require.NoError(t, err)
		customerID2, err := createTestCustomer(ctx, tx)
		require.NoError(t, err)
		orderID, err := createTestOrder(ctx, tx, customerID1)
		require.NoError(t, err)
