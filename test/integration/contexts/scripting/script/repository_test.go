...
func TestScriptRepository_CRUD(t *testing.T) {
	...
	testDB.WithTransaction(t, func(ctx context.Context, tx *sqlx.Tx) {
		...
		assert.Equal(t, script.ScriptStatusActive, updated.Status)
...
func TestScriptRepository_List(t *testing.T) {
	...
	testDB.WithTransaction(t, func(ctx context.Context, tx *sqlx.Tx) {
		...
		// List draft scripts
		draftScripts, total, err := repo.List(ctx, script.ScriptStatusDraft, 10, 0)
		require.NoError(t, err)
...
func TestScriptRepository_Execution(t *testing.T) {
	...
	testDB.WithTransaction(t, func(ctx context.Context, tx *sqlx.Tx) {
		...
		exec := script.NewScriptExecution(s.ID, s.Name, executedBy)
		...
		exec2 := script.NewScriptExecution(s.ID, s.Name, executedBy)
...
func TestScriptRepository_Pagination(t *testing.T) {
	...
	testDB.WithTransaction(t, func(ctx context.Context, tx *sqlx.Tx) {
		...
		// Page 1 (5 items)
		page1, total, err := repo.List(ctx, script.ScriptStatusActive, 5, 0)
		require.NoError(t, err)
		assert.GreaterOrEqual(t, total, 10)
		assert.LessOrEqual(t, len(page1), 5)

		// Page 2 (5 items)
		page2, _, err := repo.List(ctx, script.ScriptStatusActive, 5, 5)
		require.NoError(t, err)
