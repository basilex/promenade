...
func TestMustGetClaims(t *testing.T) {
	...
	t.Run("gets claims successfully", func(t *testing.T) {
		userID := uuidv7.New()
		email := "test@example.com"
		tokenPair, _ := manager.GenerateTokenPair(userID, email, []string{"user"})

		req := httptest.NewRequest("GET", "/must-claims", nil)
		req.Header.Set("Authorization", "Bearer "+tokenPair.AccessToken)
		w := httptest.NewRecorder()
	...
	t.Run("panics when no claims", func(t *testing.T) {
		router2 := gin.New()
		router2.Use(gin.Recovery()) // Add recovery middleware to catch panic
		router2.GET("/no-auth", func(c *gin.Context) {
			defer func() {
				if r := recover(); r != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"panic": r})
				}
			}()
			_ = MustGetClaims(c)
		...
		assert.Contains(t, w.Body.String(), "JWT claims not found in context")
	})
}
...
func TestMustGetUserID(t *testing.T) {
	...
	t.Run("gets user ID successfully", func(t *testing.T) {
		...
		req := httptest.NewRequest("GET", "/must-user-id", nil)
		req.Header.Set("Authorization", "Bearer "+tokenPair.AccessToken)
		w := httptest.NewRecorder()
	...
	t.Run("panics when no user ID", func(t *testing.T) {
		router2 := gin.New()
	...
	t.Run("panics on invalid UUID", func(t *testing.T) {
		router2 := gin.New()
		router2.Use(gin.Recovery())
		router2.GET("/invalid-uuid", func(c *gin.Context) {
			defer func() {
				if r := recover(); r != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"panic": r})
				}
			}()
			// Set invalid user ID in context
			c.Set(UserIDContextKey, "invalid-uuid")
			_ = MustGetUserID(c)
			c.JSON(http.StatusOK, gin.H{"message": "should not reach here"})
		})
