...
type Event interface {
	// Type returns the event type identifier (e.g., "user.registered", "country.created")
	Type() string
...
// Config holds common configuration for all bus implementations.
type Config struct {
...
// NewConfig creates a bus configuration with sensible defaults.
func NewConfig(workerPoolSize, bufferSize, retryAttempts int, retryDelay, retryMaxDelay time.Duration, retryMultiplier float64) Config {
	return Config{
